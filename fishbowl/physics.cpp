...
void physics::remove_force(force_id id) throw(std::invalid_argument)
{
    if (!has_force(id)) {
...
engine_id physics::add_engine(entity_id id, engine* e) throw(std::invalid_argument)
{
    assert(e != nullptr);
    if (!w.has_entity(id)) {
...
void physics::remove_engine(engine_id id) throw(std::invalid_argument)
{
    if (!has_engine(id)) {
        throw std::invalid_argument("No engine with given ID.");
    }
