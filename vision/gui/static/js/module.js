...
function sort_images() {
    ...
    for (var i = 1; i < images.length; ++i) {
        var j = i;
        while (j > 0 && images[j].getAttribute('data-index') < images[j-1].getAttribute('data-index')) {
            var tmp = images[j-1];
            images[j-1] = images[j];
            images[j] = tmp;
            --j;
...
function sort_options() {
    ...
    for (var i = 1; i < options.length; ++i) {
        var j = i;
        while (j > 0 && options[j].getAttribute('data-index') < options[j-1].getAttribute('data-index')) {
            var tmp = options[j-1];
...
$(document).ready(function(){
    ...
    socket.on('image', function(msg) {
        ...
        if (imagesContainer.find("img#" + js_name).length === 0) {
            imagesContainer.append("<li class=\"list-group-item col-xs-6\" data-index=\"" + msg.image_index + "\"><img id=\"" + js_name + "\" src=\"\" class=\"posted\"><br>" +
                                  msg.image_name + "</li>");
            sort_images();
        }
