...
opts = [
    ...
    IntOption('table block size', constants.table_block_size, 0, 4000),
    ...
    DoubleOption('table min fill ratio', constants.table_min_fill_ratio, 0, 1),
...
class Recovery(ModuleBase):
    ...
    def table(self, mat):
        ...
        candidate_tables = []
        TableInfo = namedtuple('Table', ['contour', 'hull', 'hull_area'])
        if len(contours) > 0:
            for contour in contours:
                if cv2.contourArea(contour) < self.options['table min area']:
                    continue
                if self.fill_ratio(mat, contour, adapted) < self.options['table min fill ratio']:
                    continue
                hull = cv2.convexHull(contour)
                candidate_tables.append(TableInfo(
                    contour,
                    hull,
        ...
        if len(candidate_tables) > 0:
            best_table = max(candidate_tables, key=lambda x: x.hull_area)
            cv2.fillConvexPoly(table_mask, best_table.hull, 255)
            self.results.table_visible = True
            self.results.table_x, self.results.table_y = self.contour_center(best_table.contour)
            self.results.table_area = best_table.hull_area
        ...
        for mark in threshed_marks:
            ...
            if len(contours) > 0:
                ...
                if max_hull.hull_area / table_area >= self.options['min mark table area ratio']:
                    ...
                    center = self.contour_center(max_hull.contour)
    ...
    def tower(self, mat, mask):
        ...
        for color, threshed in threshed_colors:
            ...
            for c in contours:
                ...
                if area < self.options['min stack area']:
                    continue

                # Don't detect poles of the tower
                min_rect = cv2.minAreaRect(c)
                if min(min_rect[1]) < self.options['min stack width']:
                    continue

                if self.fill_ratio(mat, c, masked) < self.options['min stack fill ratio']:
                    continue
