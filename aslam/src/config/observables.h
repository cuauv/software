...
auto NavigateObs = observationFromNavigationGroup<navigate_results, s::ratio<1, 6>>;
...
#define ASLAM_MAKE_OBSERVABLES()\
  ...
  ASLAM_MAKE_OBSERVABLE("navigation", navigate_results, NavigateObs, 0.02);\
...
#define repeat_out(macro) \
  ...
  macro(aslam_pipe_to_navigation);\
  macro(aslam_navigation);\
  macro(aslam_bin_one);\
  macro(aslam_bin_two);\
