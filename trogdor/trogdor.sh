...
if [ "$SUBMARINE" = "thor" ]; then
  SERVICES=(aslamd seriald gx4d dvld kalmand navigated controld3 shmserver ueye logging sonard visiongui cameras webgui hydromathd modules)
elif [ "$SUBMARINE" = "loki" ]; then
  SERVICES=(aslamd seriald gx1d kalmand navigated controld3 shmserver logging visiongui cameras webgui modules linearizerd)
else
