...
public:
    ...
    void remove_force(force_id id) throw(std::invalid_argument);

    engine_id add_engine(entity_id id, engine* engine) throw(std::invalid_argument);
    bool has_engine(engine_id id);
    void remove_engine(engine_id id) throw(std::invalid_argument);
